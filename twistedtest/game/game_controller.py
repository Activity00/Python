...
class GameController(object):
    ...
    @require_lock(lambda o: 'game_action_lock:' + str(o.room.room_id))
    def _start_with_lock(self):
        self.game_state.reset_state()
        success, error = self.game_state.start(self.room.get_has_position_players())
        if success:
    ...
    def start(self):
        success, error = self._start_with_lock()
        if success:
    ...
    def broadcast_new_round(self):
        ...
        # 去掉金额为0的side_pot
        side_pot = self.game_state.side_pot.copy()
        for index, pot in enumerate(side_pot):
            if pot[0] == 0:
                side_pot.pop(index)
    ...
    def broadcast_game_over(self):
        ...
        for player in self.game_state.final_players:
            player_info = object_to_dict(player, ['player_id', 'position', 'hand'])
