...
class PlayerState(object):
    def __init__(self, player_id, position, stack):
        # 进入游戏的玩家状态，可能的状态为：
        # waiting: 未行动
        # small-blind, big-blind, under-gun: 小盲，大盲, 大盲下一位
...
class GameState(object):
    ...
    def start(self, players, ante=0, limit=20, is_straddled=False):
        if self.state != 'init':
        ...
        # 强制straddle， 大盲顺时针的下一个位置， 不允许弃牌且必须下注2倍大盲
        if is_straddled and self.active_player_count >= 4:
            under_gun_position = self.active_player_positions[(self.active_player_positions.index(self.dealer_position) + 3) %
                                                   self.active_player_count]
            under_gun_player = self.active_players[self.active_player_positions.index(under_gun_position)]

            if under_gun_player.stack >= 2 * self.limit:
                self.round_bet[under_gun_position] = 2 * self.limit
                under_gun_player.stack -= 2 * self.limit
                under_gun_player.state = 'under-gun'
            else:
                self.round_bet[under_gun_position] = under_gun_player.stack
                under_gun_player.stack = 0
                under_gun_player.state = 'all-in'
                self.round_all_in.append(under_gun_position)

            self.current_position = self.get_next_position(under_gun_position)
        else:
